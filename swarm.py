...
class TopologicalDistillation:
    ...
    def build_anomaly_vector(telemetry: dict, free_energy: float) -> np.ndarray:
        ...
        vec = np.array(
            [heartbeat, flux, free_energy / 1000.0, hb_d, flux_d, severity], dtype=np.float32
        )
        return vec
...
class ResinSkill:
    ...
    def to_resin(self) -> str:
        ...
        resin = f"""skill MorphogeneticImmuneResponse {{
  version:      "1.0.0"
...
class FAISSMemory:
    ...
    def store(self, anomaly_vector: np.ndarray, skill: ResinSkill):
        ...
        print(f"  [FAISS] Stored skill '{skill.skill_id}' | Index size: {self.index.ntotal}")

    def query(self, anomaly_vector: np.ndarray, top_k: int = 1, distance_threshold: float = 50.0):
        """
...
class QueenNode:
            ...
            try:
                if self.pull_socket.poll(100):
                    ...
                    if "signature_b64" in envelope and "node_pubkey" in envelope:
                        ...
                        if CryptoEnclave.verify(
                            pub_key, envelope["signature_b64"], payload_json.encode("utf-8")
                        ):
        ...
        if status == "VALID":
            ...
            if existing_skill is not None:
                ...
                if new_reduction <= existing_reduction * 1.20:
                    self.rejected += 1
                    print(
                        f"  [QUEEN] ⚡ EUREKA COLLISION: Redundant mutation discarded. "
                        f"New ({new_reduction:.1f}) is not >20% better than existing ({existing_reduction:.1f})."
                    )
                    return
                else:
                    print(
                        f"  [QUEEN] ⚡ EUREKA COLLISION: Upgraded mutation accepted! "
            ...
            self.rejected += 1
            print("  [QUEEN] ❌ Mutation REJECTED | Insufficient FE reduction")
    ...
    def _execute_tombstone(self, compromised_pubkey: str, mutation_id: str):
        ...
        payload = {"payload": tombstone_json, "signature_b64": signature}
...
class ForagerNode:
                ...
                if self.sub_socket.poll(100):
                    ...
                    if topic == b"RESIN_SKILL":
                        ...
                        if not skill.signature or not CryptoEnclave.verify(
                            self.queen_pub_key, skill.signature, signable_content.encode("utf-8")
                        ):
                            print(
                        ...
                        print(
                            f"  [{self.node_id}] 📥 Verified Ed25519 signature! Injected skill {skill.skill_id} into local registry."
                        )
                        ...
                        if CryptoEnclave.verify(
                            self.queen_pub_key, signature, payload_json.encode("utf-8")
                        ):
    ...
    def check_and_download_skill(self, telemetry: dict, free_energy: float) -> bool:
        ...
        if skill is not None:
            print(
                f"  [{self.node_id}] 📥 Local registry match found! "
                f"(distance={distance:.2f}) → Applying topology patch..."
            )
            applied_slot = TopologicalDistillation.apply_delta(self.agent.arena, skill.delta)
            if applied_slot is not None:
                self.skills_downloaded += 1
                print(
                    f"  [{self.node_id}] ✅ Skill applied at slot {applied_slot}. "
                    f"Instant immunity — no pain required."
                )
                return True
        return False

    def notify_morphogenesis(
        self, new_slot: int, pre_fe: float, post_fe: float, anomaly_telemetry: dict
    ):
        if self.offline:
            print(f"\n  [{self.node_id}] 🌐 OFFLINE: Split-Brain Sovereignty engaged!")
            print(
                f"  [{self.node_id}] ⚠️  Clamping Z3 physical torque output to conservative 1.0 N*m for unverified mutation."
            )
            return
        ...
        print(
            f"  [{self.node_id}] Mutation '{delta['mutation_id'][:8]}' "
...
def run_swarm_demo():
    ...
    # ── Boot Forager Alpha ────────────────────────────────────────────────────
    agent_a = MorphogeneticAgent(max_capacity=MAX_ARENA_CAPACITY, initial_capacity=INITIAL_CAPACITY)
    ...
    agent_b = MorphogeneticAgent(max_capacity=MAX_ARENA_CAPACITY, initial_capacity=INITIAL_CAPACITY)
    ...
    for tick, (phase, telemetry) in enumerate(SCENARIOS_ALPHA):
        ...
        if phase == "CRISIS" and agent_a.sustained_pain_timer == 0:
            found = forager_a.check_and_download_skill(telemetry, peak_crisis_fe or 200.0)
            if found:
                phase = "IMMUNE"
        ...
        _ = extract_belief_snapshot(svi, svi_a)
        fe = float(total_loss)
        ...
        print(
            f"  Alpha Tick {tick + 1:02d} | {status} | "
            f"FE: {fe:>10.2f} | "
    ...
    print(
        f"[QUEEN]      Validated: {queen_stats['validated']} | "
    ...
    for tick, (phase, telemetry) in enumerate(SCENARIOS_BETA):
        ...
        _ = extract_belief_snapshot(svi, svi_b)
    ...
    forager_b.notify_morphogenesis(
        new_slot=2, pre_fe=900.0, post_fe=800.0, anomaly_telemetry=telemetry_c
    )

    # Alpha sends a massive crisis mutation (Pre_FE=3500, Reduction=2500)
    forager_a.notify_morphogenesis(
        new_slot=2, pre_fe=3500.0, post_fe=1000.0, anomaly_telemetry=telemetry_c
    )

    # Gamma sends a similar massive crisis but slightly inferior (Pre_FE=3500, Reduction=2550 -> not 20% better)
    forager_c.notify_morphogenesis(
        new_slot=2, pre_fe=3500.0, post_fe=950.0, anomaly_telemetry=telemetry_c
    )
    ...
    forager_b.notify_morphogenesis(
        ...
        pre_fe=1200.0,
        post_fe=200.0,
        anomaly_telemetry={"slp_heartbeat": 5.0, "sensory_flux": 9.9},
    )
