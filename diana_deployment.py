...
from certifier_action_space import StateLockedProtocol_Firewall
...
class CognitiveMindNode:
    ...
    def __init__(self):
        ...
        self.agent = MorphogeneticAgent(
            max_capacity=MAX_ARENA_CAPACITY, initial_capacity=INITIAL_CAPACITY
        )
    ...
    def spin_once(self, telemetry: dict[str, float], tick: int):
        ...
        print(
            f"[SENSOR IN] Temp: {telemetry['temp']:.1f}°C | VRAM: {telemetry['vram_usage']:.1f} GB"
        )
        ...
        if self.agent.arena.active_count > prev_active:
            print(
                f"  [MORPHOGENESIS] 🧠 Neurogenesis occurred! Arena capacity: {self.agent.arena.active_count}/{MAX_ARENA_CAPACITY}"
            )
        ...
        if fe > PAIN_THRESHOLD:
            ...
            print(
                f"  [CRISIS DETECTED] High free energy ({fe:.2f}). Generating exploratory intent..."
            )
            ...
            if is_safe:
                print(
                    f"  [D.I.A.N.A] ✅ Action VERIFIED SAFE. Token: {token}. Publishing to motor controllers."
                )
                if USE_REAL_ROS:
                    import json
                ...
                print(
                    "  [DREAM QUEUE] Routing vetoed intent to latent sandbox for nightly synthesis."
                )
                self.dream_queue.append(
                    {"telemetry": telemetry, "fe": fe, "vetoed_action": proposed_action}
                )
        else:
            print("  [NOMINAL] Homeostasis maintained. No physical action required.")
