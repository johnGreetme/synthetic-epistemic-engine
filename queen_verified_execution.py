...
class EdgeInternalMonologue:
    def __init__(self, endpoint="http://localhost:11434/api/generate", model="llama3:8b-instruct-q4_K_M"):
        self.endpoint = endpoint
        self.model = model

    def generate_autonomous_prompt(self, free_energy: float, vision_state: str) -> str:
        prompt = (f"System state: Forward motion blocked by {vision_state}. "
    ...
    def translate_to_semantic_query(self, raw_prompt: str) -> dict:
        print(f"  [EDGE  ] 🧠 Local LLM digesting state...")
...
class EpistemicCertifier:
    def certify(self, payload: dict) -> dict:
        ...
        if is_safe:
            print("  [CERT   ] 🛡️  Z3 Crucible returned SAT. No physical collisions or hardware violations.")
    ...
    def extract_physics_parameters(self, query: dict, certifier: EpistemicCertifier) -> dict:
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                result_payload["weight_b64"] = base64.b64encode(b"GENERATED_JAX_TENSOR").decode('utf-8')
                print(f"  [QUEEN ] 📦 JSON Payload received.")
                
                # Z3 Crucible Validation
                cert_result = certifier.certify(result_payload)
                if cert_result["is_safe"]:
                    print(f"  [QUEEN ] ✅ Payload passed Z3 Mathematical Invariants.")
                    return result_payload
                else:
                    print(f"  [QUEEN ] ❌ D.I.A.N.A. OS Z3 VETO GENERATED. Injecting semantic core feedback...")
                    
                    messages.append({"role": "assistant", "content": content})
                    messages.append({
                        "role": "user", 
                        "content": cert_result['reason']
                    })
                    print("  [QUEEN ] 🔄 Z3 UNSAT feedback injected into LLM context for recalculation.")
                    
            except Exception as e:
                print(f"  [QUEEN ] ⚠️ API or Parse Error: {e}")
                messages.append({"role": "assistant", "content": "Error generating valid JSON."})
                messages.append({"role": "user", "content": "Please ensure you output valid JSON matching the schema."})
                
        print("  [QUEEN ] 🚨 FATAL: MAX_RETRIES exceeded. Aborting generation. Forager must enter State-Locked Dormancy.")
        return None
