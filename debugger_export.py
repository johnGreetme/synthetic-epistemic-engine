...
from engine_core   import initialize_engine, extract_belief_snapshot, PAIN_THRESHOLD, PRIORS
...
def run_and_export(output_path: str = "debugger_data.json"):
    ...
    for tick, telemetry in enumerate(SCENARIOS):
        ...
        frame = {
            ...
            "status": (
                "CRISIS" if fe > PAIN_THRESHOLD
                else "MORPHOGENESIS" if morphogenesis_event
                else "NOMINAL"
            ),
        }
        frames.append(frame)
        print(f"  Tick {tick:02d} | FE: {fe:>10.2f} | "
    ...
    print(f"[EXPORT] Crisis ticks: {crisis_count} | "
          f"Morphogenesis events: {morph_count}")
